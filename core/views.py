...
class ProfileView(TemplateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if coremodels.Profile.objects.exclude(user = request.user).filter(display_name=display_name).exists():
...
class RegisterView(TemplateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if len(password) < 8:
            return HttpResponse("The new password must be at least 8 characters long", status=400)
